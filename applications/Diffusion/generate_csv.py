                ...
                if os.path.isdir(f"{root}/{e}/{model_company}"):
                    for model_name in os.listdir(f"{root}/{e}/{model_company}"):
                        if os.path.isdir(f"{root}/{e}/{model_company}/{model_name}"):
                            for img in os.listdir(f"{root}/{e}/{model_company}/{model_name}"):
                                df.append(
                                    {
                                        "prompt": e,
...
def process_diffusiondb(root, save_dir='./data'):
    ...
    print(f"Succesfully processed DiffusionDB dataset with {len(df)} images to {save_dir}/diffusionDB.csv")
    return df
...
def process_parti(root, save_dir='./data'):
    ...
    parti['prompt'] = parti['prompt'].apply(lambda x: x.replace(" ", "_").replace(".", "")[:100])
    df = df.merge(parti, on='prompt', how='left')
    df['prompt'] = df['full_prompt']
    df.to_csv(f'{save_dir}/parti_big.csv', index=False)
    print(f"Succesfully processed Parti dataset with {len(df)} images to {save_dir}/parti_big.csv")
    return df

def process_parti_sampled(root, save_dir='./data'):
    print("Processing Parti dataset")
    df = process_diffusion_imagefolder(root)
    df = df.groupby(['prompt', 'group_name']).apply(lambda x: x.sample(n=1)).reset_index(drop=True)
    parti = pd.read_csv('applications/Diffusion/generation/parti-prompts.csv')
    ...
    print(f"Succesfully processed Parti dataset with {len(df)} images to {save_dir}/parti.csv")
    return df

if __name__ == "__main__":
    # import data_paths from parent dir 
    sys.path.append(os.path.dirname(os.path.dirname(os.path.abspath(__file__))))
    from data_paths import PARTI_PATH, DIFFUSIONDB_PATH, CSV_SAVE_DIR
